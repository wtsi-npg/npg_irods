...
Readonly::Scalar my $DATA_LEVEL_TWO         =>
  $WTSI::NPG::HTS::PacBio::AnalysisPublisher::DATA_LEVEL;
...
sub _publish_deplexed_files {
  ...
  my $publisher =
    WTSI::NPG::HTS::PacBio::AnalysisPublisher->new(@init_args);
...
sub publish_image_archive {
  ...
  if ($self->api_client) {
    ...
    if ($metadata->has_subreads_uuid) {
      ...
      my $iap = WTSI::NPG::HTS::PacBio::ImageArchive->new(@p_init);
      push @i_handles, $iap;
      ...
      if ($metadata->has_ccsreads_uuid) {
        ...
        my $ias = WTSI::NPG::HTS::PacBio::ImageArchive->new(@s_init);
        push @i_handles, $ias;
      }
    }
...
sub _read_metadata {
  ...
  my $metadata =
    WTSI::NPG::HTS::PacBio::MetaXMLParser->new->parse_file

...

{
  ...
  sub publish_files {
    ...
    $self->info(sprintf q[Found %d Saphyr results to publish],
                        scalar @results);
    ...
    foreach my $result (@results) {
      ...
      try {
        ...
        my @flowcell_records =
            $self->find_bmap_flowcells($result->chip_serialnumber,
                                       $result->flowcell);

        my @primary_avus  = $self->make_primary_metadata($result);
        my @secondary_avus = $self->make_secondary_metadata(@flowcell_records);
